...
func TestOpenSourceFile(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			desc:       "heuristically strips absolute prefix",
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			if file, err := openSourceFile(tc.path, tc.searchPath, tc.remotePath); err != nil && tc.wantPath != "" {
				t.Errorf("openSourceFile(%q, %q, %q) = err %v, want path %q", tc.path, tc.searchPath, tc.remotePath, err, tc.wantPath)
			} else if err == nil {
				defer file.Close()
				gotPath := file.Name()
				if tc.wantPath == "" {
					t.Errorf("openSourceFile(%q, %q, %q) = %q, want error", tc.path, tc.searchPath, tc.remotePath, gotPath)
				} else if gotPath != tc.wantPath {
					t.Errorf("openSourceFile(%q, %q, %q) = %q, want path %q", tc.path, tc.searchPath, tc.remotePath, gotPath, tc.wantPath)
				}
			}
		})
	}
